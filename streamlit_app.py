...
st.set_page_config(page_title="Derived Algebraic Geometry (Lurie 2009)", layout="wide")
...
st.markdown('<h1 style="text-align: center;">📐 Derived Algebraic Geometry Engine</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Lurie (2009) – Massey product obstruction from derived moduli of portfolios</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧬 Derived AG")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'derived_ag_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Derived obstruction: {etf['derived_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Obstruction"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Obstruction", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2, tab3 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)", "📈 Walk-Forward Backtest"])

with tab1:
    st.header("📐 Top ETFs by Derived Obstruction (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Derived algebraic geometry** (Lurie 2009) studies deformations of moduli spaces.
        - The **Massey product obstruction** measures higher‑order coherence of the portfolio weight space.
        - High obstruction implies the ETF participates in non‑trivial derived structures – algebraic complexity that may translate to alpha.
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the derived obstruction per ETF.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
with tab3:
    ...
    - On each day, compute derived obstruction for all ETFs.
    - Select top 3 by obstruction.
...
st.sidebar.markdown("---")
st.sidebar.caption("Derived Algebraic Geometry | Lurie (2009) – Massey product obstruction")
