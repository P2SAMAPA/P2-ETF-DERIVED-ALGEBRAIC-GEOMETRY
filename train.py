...
from derived_ag import derived_ag_score
...
def rolling_walkforward_backtest(returns_df, window_days, top_n=3):
    """
    Walk-forward: on each day, compute derived AG score for each ETF,
    pick top_n, record next day return. Return per‑ETF average.
    """
    n = len(returns_df)
    sum_returns = {}
    count = {}
    for t in range(window_days, n - 1):
        ...
        scores = derived_ag_score(window)
...
def run_for_window(returns, window_days):
    ...
    raw_scores = derived_ag_score(ret_window)
    norm_scores = normalize_scores(raw_scores)
    sorted_norm = sorted(norm_scores.items(), key=lambda x: x[1], reverse=True)
    top_etfs = [{"ticker": t, "derived_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    for uni_name in config.UNIVERSES.keys():
        ...
        # Backtest each window (walk-forward)
        ...
        # Best window by average backtest return
        best_avg = -np.inf
        best_window = None
        best_data = None
        for wdata in all_window_results:
    ...
    out_file = f"output/derived_ag_{timestamp}.json"
